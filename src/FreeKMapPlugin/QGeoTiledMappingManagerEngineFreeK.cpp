...
#include "FreeKMapPlugin/MapProvider.h"
...
#include  <QByteArray>
#define   MAXDISKUSAGE (100 * 1024 * 1024)
//-----------------------------------------------------------------------------
        ...
        ,tileCache(nullptr)
{
     ...
     QHashIterator<QString, MapProvider*> i = getMapEngine()->freeKUrlFactory()->getProviderTable();
     while(i.hasNext()){
         i.next();
         if(!i.value()->_isElevationProvider()){
            mapList.append(QGeoMapType(i.value()->getMapStyle(), i.key(), i.key(),
                                      false,
                                      false,
                                       getMapEngine()->freeKUrlFactory()->getIdFromType(i.key()),
                                      QByteArray("freekmap"),capabilities));
         }
     }
     ...
     tileCache = new QGeoFileTileCache(cacheDirctory);
     tileCache->setMaxDiskUsage(MAXDISKUSAGE);
