...
public:
    JasonMonger();
    QSet<int> json2FreeKeoCodeType(const QJsonArray &types);
private:
...
QSet<int> JasonMonger::json2FreeKeoCodeType(const QJsonArray &types) {
    QSet<int> result;
...
QGeoCodeReplyFreeK::QGeoCodeReplyFreeK(QNetworkReply *reply, QObject *parent)
:   QGeoCodeReply(parent), m_reply(reply)
{
    connect(m_reply, &QNetworkReply::finished, this, &QGeoCodeReplyFreeK::networkReplyFinished);
...
QGeoCodeReplyFreeK::~QGeoCodeReplyFreeK()
{
    if (m_reply)
        m_reply->deleteLater();
}

void QGeoCodeReplyFreeK::abort()
{
    if (!m_reply)
        return;
...
void QGeoCodeReplyFreeK::networkReplyFinished()
{
    if (!m_reply)
        return;
    ...
    for (int i=0; i<results.size(); ++i) {
        ...
        if (geocode.contains(QStringLiteral("address_components"))) {
            ...
            for (int j=0; j<ac.size(); ++j) {
                ...
                QSet<int> types = kMonger.json2FreeKeoCodeType(c[QStringLiteral("types")].toArray());
...
void QGeoCodeReplyFreeK::networkReplyError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
    if (!m_reply)
        return;
