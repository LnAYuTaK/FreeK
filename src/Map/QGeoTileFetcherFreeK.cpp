...
void QGeoTileFetcherFreeK::_getSessionToken()
{
    ...
    QNetworkReply *sessionReply = _networkManager->get(netRequest);
    ...
    QJsonValue status = objects.value(QStringLiteral("session"));
    qDebug() << status.toString().toLatin1().data();
}
